...
from transformers import get_linear_schedule_with_warmup, get_cosine_schedule_with_warmup, BertModel
...
def build_optimizer(args, model):
    ...
    optimizer = torch.optim.AdamW(
        optimizer_grouped_parameters, 
        lr=args.adam_lr, 
        eps=args.adam_eps if args.adam_eps else 1e-6  # 默认优化
    )
...
class FreeLB(object):
    ...
    def attack(self, model, inputs, gradient_accumulation_steps=1):
        ...
        for astep in range(self.adv_K):
            ...
            loss, accuracy, pred_label, label, _ = model(inputs)
            accuracy = accuracy.mean()
        ...
        return loss, accuracy, pred_label, label
