...
def load_dataloaders(args):
    ...
    with open(os.path.join(base_url, 'train.json'), 'r', encoding='utf8') as f:
        train_data = json.load(f)

    with open(os.path.join(base_url, 'val.json'), 'r', encoding='utf8') as f:
        val_data = json.load(f)
    
    text_list = [i['reviewtext'] for i in train_data]
    label_list = [i['sentiment'] for i in train_data]
    label_list = LabelEncoder().fit_transform(label_list)
    train_dataset = BERTDataset(args, text_list = text_list, label_list = label_list)

    text_list = [i['reviewtext'] for i in val_data]
    label_list = [i['sentiment'] for i in val_data]
    label_list = LabelEncoder().fit_transform(label_list)
